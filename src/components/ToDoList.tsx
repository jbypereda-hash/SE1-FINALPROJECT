    ...
    <>
      <div className="w-full h-full overflow-x-hidden">
        ...
        <div className="bg-black-34 rounded-[24px] p-6 w-full h-full flex flex-col"> 
          ...
          {/* SCROLLABLE LIST */}
          <div className="flex-1 overflow-y-auto flex flex-col gap-3 pr-1 pb-1">
            <AnimatePresence>
              {todos.length === 0 ? (
                <motion.div
                  key="empty"
                ...
                todos.map((t) => (
                  <motion.button
                    ...
                    whileHover={{
                      backgroundColor: t.completed ? "#cfcfcf" : "#17181d",
                    }}
                    transition={{ duration: 0.15 }}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors
                      ${t.completed ? "bg-[#d9d9d9]" : "bg-[#111216]"}`}
                  >
                    {/* CHECKBOX */}
                    <div
                      ...
                      ${
                        t.completed
                          ? "bg-shrek"
                          : "border-2 border-shrek bg-transparent"
                      }`}
                    >
                      {t.completed && (
                        <svg
